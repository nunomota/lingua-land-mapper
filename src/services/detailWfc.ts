  ...
  return Array.from({ length: tileMap.length }, (_, y) =>
    Array.from({ length: tileMap[0]!.length }, (_, x) => ({
      ...
      possibilities: new Set(
        allDetailIds.filter(
          (id) => id === EMPTY || overlapRules.get(id)?.has(tileMap[y]![x]!)
        )
      ),
    }))
  );
...
function collapseWeights(
  ...
  for (const id of cell.possibilities) {
    ...
    for (const n of collapsedNeighbours) {
      const nId = grid[n.y]![n.x]!.detailId!;
...
export async function runDetailWFC({
  ...
  const allDetailIds = Array.from(adjacencyMatrix.keys());
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    ...
    const seedWeights = collapseWeights(seedCell, seedX, seedY, grid, width, height, adjacencyMatrix);
    const seedDetailId = weightedSample(seedCell.possibilities, seedWeights);
    ...
    while (collapsedCount < total) {
      ...
      for (const { x, y } of frontierMap.values()) {
        ...
        const e = entropy(cell.possibilities, adjacencyMatrix) + Math.random() * 1e-6;
        if (e < minEntropy) {
          minEntropy = e;
          candidates = [{ x, y }];
      ...
      // Collapse
      const weights = collapseWeights(cell, x, y, grid, width, height, adjacencyMatrix);
      const detailId = weightedSample(cell.possibilities, weights);
      cell.collapsed = true;
      ...
      while (queue.length > 0) {
        ...
        for (const id of Array.from(current.possibilities)) {
          const hasValidTransition = collapsedNeighbours.every((n) => {
            const nId = grid[n.y]![n.x]!.detailId!;
            const row = adjacencyMatrix.get(nId);
